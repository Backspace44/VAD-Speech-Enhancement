...
from src.tools.realtime.common import describe_device, rms_dbfs
...
class LiveComparisonUI:
    ...
    def __init__(
        ...
        self.pending_file_play_toggle = False
        ...
        self.file_play_var = tk.StringVar(value="Pause Files")
        ttk.Button(files_tab, textvariable=self.file_play_var, command=self._on_file_play_clicked, style="Primary.TButton").grid(row=3, column=0, sticky="ew", pady=(0, 10))
        self.speech_file_var = tk.StringVar(value="Speech: None")
        self.noise_file_var = tk.StringVar(value="Noise: None")
        self.file_play_status_var = tk.StringVar(value="Playback: playing")
        ttk.Label(files_tab, textvariable=self.file_play_status_var, style="Status.TLabel", wraplength=340, justify="left").grid(row=4, column=0, sticky="w", pady=(6, 4))
        ttk.Label(files_tab, textvariable=self.speech_file_var, style="Status.TLabel", wraplength=340, justify="left").grid(row=5, column=0, sticky="w", pady=(6, 4))
        ttk.Label(files_tab, textvariable=self.noise_file_var, style="Status.TLabel", wraplength=340, justify="left").grid(row=6, column=0, sticky="w")
    ...
    def set_source_mode(self, source_mode: str) -> None:
        ...
        self.pending_source_mode = source_mode
        self.source_var.set(source_mode)
    ...
    def _on_file_play_clicked(self, _event=None) -> None:
        self.pending_file_play_toggle = True
    ...
    def consume_file_play_toggle(self) -> bool:
        if self.pending_file_play_toggle:
            self.pending_file_play_toggle = False
            return True
        return False
    ...
    def set_file_playing(self, playing: bool) -> None:
        self.file_playing = bool(playing)
        self.file_play_var.set("Pause Files" if self.file_playing else "Play Files")
        self.file_play_status_var.set("Playback: playing" if self.file_playing else "Playback: paused")
    ...
    def refresh(self, stats: StreamStats, method: str) -> None:
        ...
        self.vad_ax.set_title(f"Live VAD Confidence  |  mean {float(np.mean(vad_soft)):.2f}")

        raw_level = np.clip(raw_db, -60.0, 0.0)
        out_level = np.clip(out_db, -60.0, 0.0)
        using_file_source = self.current_source_mode == "speech+noise"
        ...
        self.files_var.set(
            "input={input_dev} | output={output_dev} | files={playback} | speech_file={speech} | noise_file={noise}".format(
                input_dev=describe_device(self.current_input_device, "input"),
                output_dev=describe_device(self.current_output_device, "output"),
                playback="playing" if self.file_playing else "paused",
        ...
        if using_file_source:
            signal_text = "FILE SOURCE ACTIVE" if signal_present else "FILE SOURCE SILENT"
            signal_color = "#2f6b3b" if signal_present else "#a94442"
            input_title = "File/Synthetic Input"
        else:
            signal_text = "MIC SIGNAL DETECTED" if signal_present else "NO MIC SIGNAL"
            signal_color = "#2f6b3b" if signal_present else "#a94442"
            input_title = "Raw Microphone Input"
        self.signal_var.set(signal_text)
        self.signal_label.configure(foreground=signal_color)
        self.ax_raw.set_title(f"{input_title}  |  RMS {raw_db:.1f} dBFS  |  {signal_text}")
        self.ax_out.set_title(f"Enhanced Speaker Output  |  RMS {out_db:.1f} dBFS")
        now = time.time()
        if now - self.last_refresh_time >= 0.08:
            self.canvas.draw_idle()
            self.vad_canvas.draw_idle()
