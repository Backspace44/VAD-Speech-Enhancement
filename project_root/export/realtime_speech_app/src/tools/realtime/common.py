...
def resolve_default_checkpoint() -> Path:
    ...
    candidates = [
        config.CHECKPOINTS_DIR / "librispeech_soft_vad_recommended_smoke" / "masknet_best.pth",
        config.CHECKPOINTS_DIR / "librispeech_fast_benchmark_smoke" / "masknet_best.pth",
...
class FileDemoSource:
    ...
    def toggle_file_playing(self) -> bool:
        with self.lock:
            self.file_playing = not self.file_playing
            return self.file_playing
    ...
    def get_status(self) -> dict[str, str]:
        with self.lock:
            return {
                ...
                "file_playing": "playing" if self.file_playing else "paused",
            }
