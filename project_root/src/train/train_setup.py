...
def validate_dataset_paths(dataset_name: str):
    ...
    if dataset_name == "voicebank":
        required_paths = [
            config.VOICEBANK_ROOT / "train" / "clean",
            config.VOICEBANK_ROOT / "train" / "noisy",
            config.VOICEBANK_ROOT / "test" / "clean",
            config.VOICEBANK_ROOT / "test" / "noisy",
        ]
    elif dataset_name == "librispeech":
        required_paths = [
            config.LIBRISPEECH_ROOT / "clean" / "train" / "train-clean-100",
            config.LIBRISPEECH_ROOT / "noisy" / "train",
            config.LIBRISPEECH_ROOT / "clean" / "val" / "dev-clean",
            config.LIBRISPEECH_ROOT / "noisy" / "val",
        ]
    else:
...
def resolve_dataset_dirs(dataset_name: str, logger):
    ...
    if dataset_name == "voicebank":
        train_clean_dir = config.VOICEBANK_ROOT / "train" / "clean"
        train_noisy_dir = config.VOICEBANK_ROOT / "train" / "noisy"
        val_clean_dir = config.VOICEBANK_ROOT / "test" / "clean"
        val_noisy_dir = config.VOICEBANK_ROOT / "test" / "noisy"
    elif dataset_name == "librispeech":
        train_clean_dir = config.LIBRISPEECH_ROOT / "clean" / "train" / "train-clean-100"
        train_noisy_dir = config.LIBRISPEECH_ROOT / "noisy" / "train"
        val_clean_dir = config.LIBRISPEECH_ROOT / "clean" / "val" / "dev-clean"
        val_noisy_dir = config.LIBRISPEECH_ROOT / "noisy" / "val"
