...
def resolve_split_dirs(split: str) -> tuple[Path, Path]:
    ...
    return (
        ...
        config.LIBRISPEECH_ROOT / "noisy" / "test",
    )
...
def build_dataset(
    ...
    clean_dir, noisy_dir = resolve_split_dirs(split)
...
def summarize_labels(vad_path: Path | None, noisy_stem: str) -> dict[str, float]:
    if vad_path is None:
        return {
            "label_mean": 1.0,
            "label_std": 0.0,
    ...
    label = np.load(vad_path / f"{noisy_stem}.npy")
...
def analyze_dataset(
    ...
    for idx in range(len(dataset)):
        item = dataset[idx]
        noisy_stem = dataset.noisy_files[idx].stem
        row: dict[str, float | str] = {
            "setting": name,
            "filename": item["filename"],
            "noisy_stem": noisy_stem,
        }
        row.update(summarize_mask(item["ideal_mask"]))
        row.update(summarize_labels(vad_path, noisy_stem))
        rows.append(row)

    numeric_keys = [key for key in rows[0].keys() if key not in {"setting", "filename", "noisy_stem"}]
    summary = {
